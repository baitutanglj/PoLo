        ...
        for f in fact_files:
            with open(self.input_dir + f + '.txt') as raw_input_file:
                ...
                for line in csv_file:
                    p = [self.entity_vocab[l] if idx % 2 == 0 else self.relation_vocab[l] for idx, l in
                         enumerate(line[3].split('|'))]
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            y = torch.tensor(list(self.data.store_all_correct[el]))
            return x, y
...
def mycollate_fn_test(data):
    #这里的data是getittem返回的（x，y）的二元组，总共有batch_size个
    x_batch = torch.stack([i[0] for i in data])
    y_batch = [i[1] for i in data]
    y_batch = pad_sequence(y_batch, batch_first=True, padding_value=0)
...
if __name__ == "__main__":
    ...
    #     print('x:', x.shape)
    #     print('y:', y.shape)
    train_data = MyDataset(args, mode="test")
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, collate_fn=mycollate_fn_test)
