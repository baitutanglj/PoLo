import numpy as np
import sys
sys.path.append("/mnt/home/linjie/projects/KG/PoLo/")
from mycode_one_answer.data.grapher import RelationEntityGrapher
from mycode_one_answer.data.feed_data import RelationEntityBatcher
...
class Episode(object):
    def __init__(self, graph, data, params):
        ...
        # self.all_answers = np.repeat(all_answers, self.rollouts) #my
    ...
    # def get_rewards(self):
    #     rewards = np.array([self.current_entities[i] in self.all_answers[i] for i in range(len(self.current_entities))])
    #     # Set the True and False values to the values of positive and negative rewards
    #     condlist = [rewards == True, rewards == False]
    #     choicelist = [self.positive_reward, self.negative_reward]
    #     rewards = np.select(condlist, choicelist)   # [B,]
    ...
    def __call__(self, action):
        ...
        self.states['next_relations'] = next_actions[:, :, 1]#(3840, 400)
        self.states['next_entities'] = next_actions[:, :, 0]#(3840, 400)
        self.states['current_entities'] = self.current_entities#(3840,)
        return self.states
...
class Env(object):
    def __init__(self, params, mode='train'):
        ...
        # self.grapher_small = RelationEntityGrapher(triple_store=input_dir + 'graph_small.txt',
        #                                            entity_vocab=params['entity_vocab'],
        #                                            relation_vocab=params['relation_vocab'],
        #                                            max_num_actions=params['max_num_actions'])
        # self.grapher = RelationEntityGrapher(triple_store=input_dir + 'graph_big.txt',
        #                                      entity_vocab=params['entity_vocab'],
        #                                      relation_vocab=params['relation_vocab'],
